...
def search_openalex(query: str) -> str:
    ...
    try:
        ...
        formatted_results.append(
            f"Found {data.get('meta', {}).get('count', 0)} papers (showing top 5)\n"
        )
        ...
        for i, work in enumerate(results, 1):
            ...
            # PDF URL
            pdf_url = work.get("open_access", {}).get("is_oa", False) and work.get(
                "best_oa_location", {}
            ).get("pdf_url")
            ...
            formatted_results.append(
                f"  📅 Year: {publication_year} | 📢 Citations: {cited_by_count}"
            )
            if abstract:
