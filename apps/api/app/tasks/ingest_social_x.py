"""X (formerly Twitter) source ingestion task."""
...
async def _ingest_x(
    *,
    ...
    try:
        ...
        tasks = [
            search_x_posts(project_id, qv, max_results=fetch_limit, redis=redis) for qv in variants
        ]
    ...
    for _, _, _, _, post in scored:
        ...
        if not sid or not content:
            continue
        author = str(post.get("author_username") or post.get("author") or "x").strip()
        docs.append(
            RawDocument(
                project_id=project_id,
                user_id=user_id,
                ...
                metadata={
                    ...
                    "likes": _as_int(post.get("likes")),
                    "views": _as_int(post.get("views")),
                    "url": post.get("url") or f"https://x.com/i/status/{sid}",
