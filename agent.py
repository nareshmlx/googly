...
def create_agent(enable_tiktok: bool, enable_openalex: bool, user_id: str, session_id: str) -> Agent:
    """
    ...
    try:
        ...
        for session in sessions:
            try:
                ...
                # Count messages
                message_count = sum(
                    len(run.messages) for run in (session.runs or [])
                )
...
def get_session_state_data(session_id: str, user_id: str) -> dict:
    """
    Get session state (including discovered videos) from the database.
    ...
    Returns:
        Dict containing session state data
    """
    db = get_database()
    if not db:
        return {}
    ...
    try:
        ...
        return agent.get_session_state(session_id=session_id) or {}
    except Exception:
        return {}


def update_session_state_data(session_id: str, user_id: str, state_updates: dict) -> dict:
    """
    Update session state (e.g., save discovered videos) in the database.
    
    Args:
        session_id: The session ID
        user_id: The user ID
        state_updates: Dict of state updates to apply
    ...
    try:
        ...
        return agent.update_session_state(
            session_id=session_id,
            session_state_updates=state_updates
        ) or {}
    except Exception:
        return {}
